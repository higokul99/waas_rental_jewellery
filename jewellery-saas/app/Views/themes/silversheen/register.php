...
<head>
  ...
  <title>Register - SILVERSHEEN</title>

  <!-- include styles used by theme -->
  ...
  <main class="auth-page">
    <div class="auth-card">
      ...
      <form action="#" method="post">
        ...
        <div class="mb-3">
          <label for="pin" class="form-label">Choose 4‑digit PIN</label>
          <input type="password" name="pin" id="pin" class="form-control" maxlength="4" pattern="\d{4}" required placeholder="****">
        </div>
        <button type="submit" class="btn btn-primary w-100">Create Account</button>
      </form>
      <p class="mt-3 text-center small">Already have an account? <a href="<?= base_url('login') ?>" class="text-decoration-underline">Log in</a></p>
    </div>
  </main>
